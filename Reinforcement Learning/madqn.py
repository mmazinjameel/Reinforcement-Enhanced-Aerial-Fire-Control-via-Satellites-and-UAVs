...
class Config(object):
    ...
    def __init__(self, config_type='train'):
        ...
        self.forest_dimension_ = 50
        self.delta_beta_ = 0.15/0.2763
        self.image_dims = (3, 3)
        self.fire_center = np.array([(self.forest_dimension_+1)/2, (self.forest_dimension_+1)/2])
        self.update_sim_every = 6

        # agent initialization locations
        self.start = np.arange(self.forest_dimension_ // 3 // 2, self.forest_dimension_, self.forest_dimension_ // 3)
        self.perturb = np.arange(-self.forest_dimension_ // 3 // 2 + 1, self.forest_dimension_ // 3 // 2 + 1, 1)
...
class MADQN(object):
    ...
    def train(self, num_episodes=1):
        ...
        sim = LatticeForest(self.config.forest_dimension_)
        num_agents = 10
                ...
                for agent in team.values():
                    ...
                    if 0 <= position_rc[0] < sim.dims[0] and 0 <= position_rc[1] < sim.dims[1]:
                        if sim.group[position_rc].is_on_fire(sim.group[position_rc].state):
                            sim_control[position_rc] = (0.0, self.config.delta_beta_)
