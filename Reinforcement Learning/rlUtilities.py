...
def actions2trajectory_(position, actions):
    """
    helper function to map a set of actions to a trajectory_.
    ...
    Returns:
    - trajectory_: list of (x, y) positions created by taking actions, where the
                  first element is the input position
    """

    trajectory_ = []
    x, y = position
    trajectory_.append((x, y))
    for a in actions:
        x, y = trajectory_[-1]
        if a == 0:
            trajectory_.append((x, y))
        elif a == 1:
            trajectory_.append((x-1, y+1))
        elif a == 2:
            trajectory_.append((x, y+1))
        elif a == 3:
            trajectory_.append((x+1, y+1))
        elif a == 4:
            trajectory_.append((x-1, y))
        elif a == 5:
            trajectory_.append((x+1, y))
        elif a == 6:
            trajectory_.append((x-1, y-1))
...
def move_toward_center(agent):
    ...
    for idx, a in enumerate([2, 5, 7, 4, 1, 3, 8, 6]):
        new_position = actions2trajectory_(agent.position, [a])[1]
    ...
    if agent.reached_fire:
        ...
        if not move_left and agent.image[ri, ci] == agent.healthy:
            ...
            if healthy_neighbors >= 6:
                circular_position = actions2trajectory_(agent.position, [right_action])[1]
                action = right_action
